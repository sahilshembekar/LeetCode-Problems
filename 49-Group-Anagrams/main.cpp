...
public:
    vector<vector<string>> groupAnagrams(vector<string>& strs) {
        ...
        for (auto str : strs) {
            ...
            string word_copy = str; //a is the word 
            sort(str.begin(), str.end()); //Directly Sorting the word 
            auto it = map1.find(str);
            if (it==map1.end()) {
                map1[str].push_back(word_copy);// The syntax is map_name[key].push_back(whatToInsert)    
            }
            else {
                // map_data[str].push_back(word_copy); --> Also works with this
                it->second.push_back(word_copy);
            }     
        }
        for (auto i : map1) {
