// https://leetcode.com/problems/spiral-matrix/
        ...
        if (top <=bottom && left <= right) { // In the above step we do top++ and we are already inside the while loop. Condition will not be checked again. Hence we need this if statement 
        // we can also use only if(top<=bottom) 
        //here we are checking (top<=bottom) because the value of top has been modified and has to satisfy our while loop constraint.
        ...
        //here we are checking (left<=right) because the value of right has been modified and has to satisfy our while loop constraint.
